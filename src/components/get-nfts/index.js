...
class GetNfts extends React.Component {
  ...
  async handleGetBalance (event) {
    try {
      ...
      this.lazyLoadTokenIcons2(nftCandidates)
  ...
  async lazyLoadTokenIcons2 (tokens) {
    for (let i = 0; i < tokens.length; i++) {
      const thisToken = tokens[i]
      console.log(`thisToken: ${JSON.stringify(thisToken, null, 2)}`)
  ...
  async lazyLoadTokenIcons (tokens) {
    ...
    for (let i = 0; i < nfts.length; i++) {
      const thisToken = nfts[i]
      // const tokenFound = false

      if (thisToken.tokenData.mutableData.tokenIcon) {
        const tokenIcon = thisToken.tokenData.mutableData.tokenIcon
    ...
    // Update the state, re-render, omitting any non-NFTs that made it through
    // the first filter.
    this.setState({
      tokens: nfts
    })
